...
- 同時比對正向彩色原圖、溫和點陣強化圖及 180° 日期圖，不依表格列位置猜測日期
- AI 低信心或點陣字無法讀取時，可用保留前後相鄰列的逐日放大圖批次快速核對並一次儲存
...
- 月薪員工提早上班加班費
- 獎金、禮金、加給與扣款
- 獎金、禮金、其他加給與扣款分組彙總，薪資表同步列出實際項目
- 品牌化四色介面與初一食午 Logo APP／PWA 安裝圖示
...
API Key 只存在 Netlify 後端，不會出現在 GitHub 或瀏覽器中。店內連線密碼只儲存在目前瀏覽器，不會納入薪資 JSON 備份。APP 會先裁掉卡片上方姓名區，再將打卡表格的正向彩色、點陣強化及 180° 日期輔助圖傳送至 Gemini；員工姓名不會放入 AI 請求。Gemini 免費層內容可能由 Google 用於改善產品，仍應由店內依個資管理規範限制網站與照片存取。
